...
const BlogsPage = () => {
    ...
    useEffect(() => {
        // Safe Check: Agar user logged in hai tabhi ID set ho, varna app crash nahi karegi
        const user = getUser();
        if (user && user._id) {
    ...
    const handleViewDetail = async (blog) => {
        // --- AUTH CHECK TRIGGER ---
        // Agar user logged in nahi hai, toh use aage mat jaane do, seedha login page bhejo
        if (!currentUserId) {
            alert("🔒 authentication required. Please login first to view blog details or purchase premium content!");
            navigate('/auth');
            return;
        }

        // --- FREE BLOG LOGIC ---
        // Agar blog paid nahi hai aur user logged in hai, toh seedha andar jaane do
        if (!blog.isPaid) {
        ...
        if (hasPaid) {
            ...
            // Stripe checkout 
            try {
                alert(`🔒 Redirecting to Stripe... Please complete the payment of $${blog.price} to access this blog content.`);
                ...
                {blogs.length === 0 ? (
                    ...
                    <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                        {blogs.map((blog) => {
                            ...
                            const isUnlocked = currentUserId && blog.purchasedBy && blog.purchasedBy.includes(currentUserId);
